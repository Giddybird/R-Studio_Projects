...
p1<-ggplot() + geom_point(data=g1, aes(x=year, y=gdpPercap, color=continent, size=pop/1000000)) + scale_x_continuous(name="Year", limits=c(1952, 2007)) +
  geom_line(g1, mapping=aes(x=year,y=gdpPercap, group=country, color=continent))+
   geom_point(data=g1, aes(x=year, y=averageGDPPercap, size=pop/1000000),color="black")+
  geom_line(data=g1, aes(x=year, y=averageGDPPercap),color="black")+
ylab("GDP Per Cap")+
xlab("Year")+
  labs(color= "Continent", size="Population, 1000K")+
